...
extension BluetoothService {
    ...
    func requestPmsetFallback(reason: String) {
        ...
        AppLogger.bluetooth.info("Triggering pmset fallback (\(reason))")
    ...
    private func mergePmsetEntries(
        ...
        if logNewEntries {
            for entry in newlyFilled {
                AppLogger.bluetooth.info("pmset reported \(entry.level)% for \(entry.displayName)")
            }
        }
    ...
    func logMissingBatteryInfo(forName name: String, address: String) {
        ...
        AppLogger.bluetooth.warning("Battery percentage unavailable for \(displayName) (\(displayAddress))")
    }
