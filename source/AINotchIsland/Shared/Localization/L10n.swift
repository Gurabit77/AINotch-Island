...
enum L10n {
    /// Returns a `LocalizedStringKey` backed by the resolved localized string.
    /// Use this overload when passing to SwiftUI views that expect `LocalizedStringKey`.
    static func appKey(_ key: String, fallback: String? = nil) -> LocalizedStringKey {
        LocalizedStringKey(stringLiteral: app(key, fallback: fallback))
    }

    static func app(_ key: String, fallback: String? = nil) -> String {
        string(
            key,
            language: AINotchIslandLanguage.resolved(
    ...
    static func string(_ key: String, language: AINotchIslandLanguage, fallback: String? = nil) -> String {
        localizedString(
            key,
