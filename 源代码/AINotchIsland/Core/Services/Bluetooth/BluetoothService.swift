...
final class BluetoothService: ObservableObject {
    ...
    private init() {
        AppLogger.bluetooth.info("Initializing...")
        setupBluetoothObservers()
        // checkInitialDevices() calls IOBluetooth synchronously, which on a
        // fresh install blocks the main thread waiting for TCC's Bluetooth
        // prompt — but the run loop hasn't started yet, so the prompt can
        // never display and the app deadlocks. Defer to the next runloop
        // tick so AppKit can finish launching first.
    ...
    func presentDeviceConnectedHUD(device: BluetoothAudioDevice, batteryLevel: Int?) {
        AppLogger.bluetooth.debug("Showing device connected HUD")
