...
final class SystemMediaKeyTap {
    ...
    func start() -> Bool {
        ...
        guard let systemDefinedEvent else {
            AppLogger.hud.error("Failed to resolve the system-defined CGEvent type.")
            return false
        }
        ...
        guard let createdTap = CGEvent.tapCreate(
            ...
            AppLogger.hud.error("Failed to create the media key event tap. Accessibility permission may be missing.")
            return false
        }
