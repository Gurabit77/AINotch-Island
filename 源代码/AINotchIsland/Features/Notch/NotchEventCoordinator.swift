...
final class NotchEventCoordinator: ObservableObject {
    ...
    private let agentHaloHandler: NotchAgentHaloEventsHandler
    ...
    init (
        ...
        self.agentHaloHandler = NotchAgentHaloEventsHandler(
            notchViewModel: notchViewModel,
            agentHaloViewModel: agentHaloViewModel,
            buddyEngine: buddyEngine
        )
    ...
    func handleAgentHaloEvent(_ event: AgentHaloEvent) {
        agentHaloHandler.handleAgentHalo(event)
    }
    ...
    func handleScreenRecordingEvent(_ event: ScreenRecordingEvent) {
        ...
        switch event {
        case .started:
            buddyEngine.showTemporaryScene(.screenRecording, duration: 3.0)
            notchViewModel.send(
                .showLiveActivity(
    ...
    func handleExternalDriveEvent(_ event: ExternalDriveEvent) {
        guard !isOnboardingActive else { return }
        guard !isLockScreenTransitionActive else { return }

        switch event {
        case .connected(let name):
            buddyEngine.showTemporaryScene(.usbConnected, duration: 2.5)
            notchViewModel.send(
                .showTemporaryNotification(
                    ExternalDriveNotchContent(
                        id: NotchContentRegistry.ExternalDrive.connected.id,
                        driveName: name,
                        isEjecting: false
                    ),
                    duration: 3.0
        ...
        case .ejected(let name):
            buddyEngine.showTemporaryScene(.usbEjected, duration: 2.0)
            notchViewModel.send(
                .showTemporaryNotification(
                    ExternalDriveNotchContent(
                        id: NotchContentRegistry.ExternalDrive.ejected.id,
                        driveName: name,
                        isEjecting: true
                    ),
                    duration: 3.0
    ...
    private let buddyScheduler = BuddyIdleScheduler()
    private let buddyEmotion = BuddyEmotionState()
    private let buddyHoverTracker = BuddyHoverTracker()
    private let buddyEnvironment = BuddyEnvironmentService()

    func registerPersistentBuddy() {
        buddyEmotion.start()
        buddyEnvironment.emotionState = buddyEmotion
        buddyEnvironment.start()
        let screenWidth = NSScreen.main?.frame.width ?? 1512
        buddyHoverTracker.start(notchCenterX: screenWidth / 2)
        notchViewModel.send(
            .showLiveActivity(
                BuddyAlwaysOnContent(
                    engine: buddyEngine,
                    scheduler: buddyScheduler,
                    emotionState: buddyEmotion,
                    hoverTracker: buddyHoverTracker,
                    environmentService: buddyEnvironment
                )
            )
        )
    }

    func showBuddyIdleIfNeeded() {
        registerPersistentBuddy()
    }

    func handleScreenshotEvent(_ event: ScreenshotEvent) {
        guard !isOnboardingActive else { return }
        guard !isLockScreenTransitionActive else { return }

        buddyEngine.showTemporaryScene(.screenshot, duration: 2.5)
        notchViewModel.send(
            .showTemporaryNotification(
                BuddyIdleNotchContent(scene: .screenshot),
                duration: 2.5
            )
        )
    }

    func handleExternalDisplayEvent(_ event: ExternalDisplayEvent) {
        guard !isOnboardingActive else { return }
        guard !isLockScreenTransitionActive else { return }

        switch event {
        case .connected:
            buddyEngine.showTemporaryScene(.mirrorDisplay, duration: 3.0)
            notchViewModel.send(
                .showTemporaryNotification(
                    BuddyIdleNotchContent(scene: .mirrorDisplay),
                    duration: 3.0
                )
            )
        case .disconnected:
