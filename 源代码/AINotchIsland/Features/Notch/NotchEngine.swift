...
final class NotchEngine: ObservableObject {
    ...
    func updateBaseGeometry(width: CGFloat, height: CGFloat, scale: CGFloat, hardwareNotchHeight: CGFloat = 0) {
        notchModel.baseWidth = width
        notchModel.baseHeight = height
        notchModel.scale = scale
        notchModel.hardwareNotchHeight = hardwareNotchHeight
    }
    ...
    func handleOutsideClick() {
        ...
        if liveActivityContent.preventsAutoCollapse { return }

        if isTransitioning {
            withAnimation(animations.contentHide) {
                notchModel.isLiveActivityExpanded = false
            }
            return
        }
    ...
    private func showLiveContentTransition(_ content: NotchContentProtocol?) async {
        ...
        if isTransitioning {
            try? await Task.sleep(nanoseconds: UInt64((hideDelay + 0.05) * 1_000_000_000))
            if notchModel.liveActivityContent?.id == content?.id { return }
