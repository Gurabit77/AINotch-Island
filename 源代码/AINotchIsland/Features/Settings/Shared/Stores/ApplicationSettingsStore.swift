...
final class ApplicationSettingsStore: SettingsStoreBase, NotchSettingsProviding {
    ...
    @Published var appLanguage: AINotchIslandLanguage {
        didSet {
    ...
    @Published var agentHaloLayoutMode: AgentHaloLayoutMode {
        didSet {
            persist(agentHaloLayoutMode.rawValue, for: GeneralSettingsStorage.Keys.agentHaloLayoutMode)
        }
    }

    @Published var agentHaloBypassPermissions: Bool {
        didSet {
            persist(agentHaloBypassPermissions, for: GeneralSettingsStorage.Keys.agentHaloBypassPermissions)
        }
    }

    @Published var agentHaloAutoMode: Bool {
        didSet {
            persist(agentHaloAutoMode, for: GeneralSettingsStorage.Keys.agentHaloAutoMode)
        }
    }

    @Published var agentHaloSmartSuppressionEnabled: Bool {
        didSet {
            persist(agentHaloSmartSuppressionEnabled, for: GeneralSettingsStorage.Keys.agentHaloSmartSuppression)
        }
    }

    @Published var agentHaloIdleCleanupTimeout: Int {
        didSet {
            persist(agentHaloIdleCleanupTimeout, for: GeneralSettingsStorage.Keys.agentHaloIdleCleanupTimeout)
        }
    }
    ...
    override init(defaults: UserDefaults) {
        ...
        self.appLanguage = AINotchIslandLanguage.resolved(
        ...
        self.agentHaloLayoutMode = AgentHaloLayoutMode(
            rawValue: defaults.string(forKey: GeneralSettingsStorage.Keys.agentHaloLayoutMode) ?? AgentHaloLayoutMode.clean.rawValue
        ) ?? .clean
        self.agentHaloBypassPermissions = defaults.bool(forKey: GeneralSettingsStorage.Keys.agentHaloBypassPermissions)
        self.agentHaloAutoMode = defaults.bool(forKey: GeneralSettingsStorage.Keys.agentHaloAutoMode)
        self.agentHaloSmartSuppressionEnabled = Self.resolvedBool(
            defaults: defaults,
            key: GeneralSettingsStorage.Keys.agentHaloSmartSuppression
        )
        self.agentHaloIdleCleanupTimeout = defaults.object(forKey: GeneralSettingsStorage.Keys.agentHaloIdleCleanupTimeout) as? Int ?? 7200
    ...
    func resetGeneral() {
        ...
        appLanguage = AINotchIslandLanguage.resolved(
    ...
    private func updateLaunchAtLogin() {
        ...
        do {
            ...
            AppLogger.general.error("Failed to update launch-at-login for \(instance.description): \(error)")
        }
    }
